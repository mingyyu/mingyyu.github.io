...
function checkForWinner() {
    // Check the rows
    for (let i = 0; i < 9; i += 3) {
        if (squares[i].textContent && squares[i].textContent === squares[i + 1].textContent && squares[i].textContent === squares[i + 2].textContent) {
            gameOver = true;
    ...
    // Check the columns
    for (let i = 0; i < 3; i++) {
        if (squares[i].textContent && squares[i].textContent === squares[i + 3].textContent && squares[i].textContent === squares[i + 6].textContent) {
            gameOver = true;
            alert(`Player ${currentPlayer} wins!`);
            return;
        }
